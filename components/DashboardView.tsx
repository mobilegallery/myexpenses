...
import { CATEGORIES, BS_MONTHS } from '../constants';
...
const DashboardView: React.FC<DashboardViewProps> = ({ expenses, onShowCategoryDetails, onShowPayerDetails, theme, onSetTheme, onStartEdit, onShowPhoto, onSync, isSyncing, isSaving, onNavigateToSettings }) => {
    ...
    const totals = useMemo(() => {
        let husbandTotal = 0;
        let wifeTotal = 0;
        const categoriesTotal: { [key in Category]: number } = { Food: 0, Rent: 0, Car: 0, Medical: 0, Personal: 0, Store: 0, Other: 0 };

        expenses.forEach(exp => {
            if (exp.payer === 'Husband') husbandTotal += exp.amount;
            else if (exp.payer === 'Wife') wifeTotal += exp.amount;
            if (categoriesTotal[exp.category] !== undefined) {
    ...
    const doughnutData = {
        labels: CATEGORIES.filter(cat => totals.categoriesTotal[cat] > 0),
        datasets: [{
            data: CATEGORIES.map(cat => totals.categoriesTotal[cat]).filter(amount => amount > 0),
            backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40', '#C9CBCF'],
            hoverBackgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40', '#C9CBCF']
        }]
    };
                                ...
                                {sortedExpenses.length > 0 ? (
                                    sortedExpenses.map(exp => (
                                        <tr key={exp.id} className={`border-b border-light-border dark:border-dark-border ${exp.payer === 'Husband' ? 'bg-husband-light/50 dark:bg-husband-dark/50' : 'bg-wife-light/50 dark:bg-wife-dark/50'}`}>
                                            ...
                                            <td className="p-4 text-left hidden sm:table-cell">{exp.category}</td>
                                    ...
                                    <>
                                        <tr className="sm:hidden">
                                            <td colSpan={5} className="p-4 text-left text-gray-500 dark:text-gray-400">No expenses recorded yet.</td>
                                        </tr>
                                        <tr className="hidden sm:table-row">
                                            <td colSpan={6} className="p-4 text-left text-gray-500 dark:text-gray-400">No expenses recorded yet.</td>
                                        </tr>
                                    </>
                                )}
                            </tbody>
